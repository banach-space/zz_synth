...
//   AR envelopes and to save them to WAVE files using the zz-synthesiser. A
//   number of files with enveloped plain sound  are generated. There are seven
//   hard-coded parameters that determine the sounds being produced. These are
//   marked with 'Param #:' and can be freely modified to experiment.
//
...
int main() {
  ...
  for (auto it = pitch.begin(); it != pitch.end(); it++) {
    // 1. Generate filename
    sprintf(file_name,
            "examples/ar_envelope_note/sounds/ar_envelope_note_%d.wav",
            file_idx);
    file_idx++;
